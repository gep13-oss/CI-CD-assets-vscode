...
Setup(context =>
{
    ...
    Information("Building version {0} of ci-cd-assets-vscode ({1}, {2}) using version {3} of Cake. (IsTagged: {4})",
...
Task("Clean")
    .Does(() =>
{
    CleanDirectories(new[] { "./BuildArtifacts" });
});
...
Task("Create-Release-Notes")
    .Does(() =>
{
    GitReleaseManagerCreate(parameters.GitHub.UserName, parameters.GitHub.Password, "gep13", "CI-CD-assets-vscode", new GitReleaseManagerCreateSettings {
...
Task("Package-Extension")
    ...
    .Does(() =>
{
    var buildResultDir = Directory("./BuildArtifacts");
    var packageFile = File("ci-cd-assets-vscode-" + parameters.Version.SemVersion + ".vsix");
    ...
    .Does(() =>
{
    ...
    var nuspecFile = File("./chocolatey/ci-cd-assets-vscode.nuspec");

    EnsureDirectoryExists(parameters.ChocolateyPackages);
    var extensionFile = MakeAbsolute((FilePath)("./BuildArtifacts/ci-cd-assets-vscode-" + parameters.Version.SemVersion + ".vsix"));
    ...
    files.Add(new ChocolateyNuSpecContent { Source = extensionFile.ToString(), Target = "tools/ci-cd-assets-vscode.vsix" });
...
.Does(() =>
{
    var buildResultDir = Directory("./BuildArtifacts");
    var packageFile = File("ci-cd-assets-vscode-" + parameters.Version.SemVersion + ".vsix");
    var chocolateyPackageFile = File("_Packages/chocolatey/ci-cd-assets-vscode." + parameters.Version.SemVersion + ".nupkg");
...
Task("Publish-GitHub-Release")
    ...
    .Does(() =>
{
    var packageFiles = GetFiles("./BuildArtifacts/*.vsix")
                     + GetFiles(parameters.ChocolateyPackages + "/*.nupkg");

    foreach (var package in packageFiles.Select(f => MakeAbsolute(f)))
    {
        GitReleaseManagerAddAssets(parameters.GitHub.UserName, parameters.GitHub.Password, "gep13", "ci-cd-assets-vscode", parameters.Version.Milestone, package.ToString());
    }

    GitReleaseManagerClose(parameters.GitHub.UserName, parameters.GitHub.Password, "gep13", "ci-cd-assets-vscode", parameters.Version.Milestone);
})
.OnError(exception =>
{
    ...
    .Finally(() =>
{
    if(publishingError)
    {
        throw new Exception("An error occurred during the publishing of ci-cd-assets-vscode.  All publishing tasks have been attempted.");
    }
});
